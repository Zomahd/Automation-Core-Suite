...
public class HeroReportListener extends IHeroListener implements IReporter {
    ...
    public HeroReportListener() {
        if(listeners == null) {
            listeners = new ArrayList<>();
            addListeners(HeroConfigWrapper.INSTANCE.get("hero.reporters",
                    "com.accenture.dpcs.hero.reporters.excel.ExcelReporter," +
                    "com.accenture.dpcs.hero.reporters.extent.ExtentReporter"));
        }
    }
    ...
    private boolean includeLatestDirectory() {
        return HeroConfigWrapper.INSTANCE.getBoolean("hero.reports.include.latest.directory");
    }
    ...
    protected void instanceError(String msg, String name, Throwable throwable) {
        HeroErrorUtils.log("HeroReportListener: " + INVALID_LISTENER + "\"" + name.trim() + "\", " + msg, throwable);
    }

    @Override
    protected void methodError(String method, ITestNGListener listener, Throwable throwable) {
        HeroErrorUtils.log("HeroReportListener: " + ERROR_WHILE_EXEC + " \"" +
