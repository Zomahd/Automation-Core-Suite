...
public class FileUtil {
    ...
    private static void copyCustomProfile() {
        ...
        File destination = new File(HeroConfigWrapper.INSTANCE.get("chrome.profile.destination"));
        try {
        ...
        if(deleted) {
            ...
            destination = new File(HeroConfigWrapper.INSTANCE.get("chrome.profile.destination"));
            destination.mkdirs();

            String source = HeroConfigWrapper.INSTANCE.get("chrome.profile.location", "");
            try {
                if (!source.isEmpty()) {
